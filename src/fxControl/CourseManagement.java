...
public class CourseManagement implements Initializable {
    ...
    public void addCourse(ActionEvent actionEvent) throws SQLException {
        ...
        String sql = "SELECT * FROM course WHERE course_name = ? AND start_date = ? AND end_date = ? AND admin_id = ? AND course_price = ? AND course_is = ?";
        statement = connection.prepareStatement(sql);
        statement.setString(1, nameField.getText());
        statement.setString(2, String.valueOf(startField.getValue()));
        statement.setString(3, String.valueOf(endField.getValue()));
        statement.setInt(4, Integer.parseInt(admidIdField.getText()));
        statement.setDouble(5, Double.parseDouble(courseIsField.getText()));
        statement.setInt(6, Integer.parseInt(courseIsField.getText()));
        ResultSet rs = statement.executeQuery();
        if (rs.next()) {
            utilOperations.alertMessage("Course already exists!");
        } else {
            PreparedStatement stmt = connection.prepareStatement("INSERT INTO course(course_name, start_date, end_date, admin_id, course_price, course_is) VALUES (?, ?, ?, ?, ?, ?)");
            stmt.setString(1, nameField.getText());
            stmt.setString(2, String.valueOf(startField.getValue()));
            stmt.setString(3, String.valueOf(endField.getValue()));
            stmt.setInt(4, Integer.parseInt(admidIdField.getText()));
            stmt.setDouble(5, Double.parseDouble(courseIsField.getText()));
            stmt.setInt(6, Integer.parseInt(courseIsField.getText()));
            stmt.executeUpdate();
